...
public class StudentServiceImpl implements StudentService {
	...
	public void inputStudent() {
		...
		while (fileReader.hasNext()) {
			...
			StudentVO stVO1 = StudentVO.builder().stName(name).stNum(num).stDept(dept).stGrade(grade).stTel(tel)
					.build();
			stList.add(stVO1);
			try {
				is.close();
			...
			while (true) {
				...
				if (read.equals("QUIT"))
					break;
				stVO1.setStName(read);
				System.out.print("학과를 입력하세요 >> ");
				read = scan.nextLine();
				if (read.equals("QUIT"))
					break;
				stVO1.setStDept(read);
				System.out.print("학년을 입력하세요 >> ");
				read = scan.nextLine();
				if (read.equals("QUIT"))
					break;
				stVO1.setStGrade(read);
